...
def add_or_get_bucket(buckets, rules: Optional[OmniarchiveRules]) -> MetaVersion:
    rule_hash = None
    if rules:
...
def is_macos_only(rules: Optional[OmniarchiveRules]):
    allows_osx = False
    allows_all = False
...
def version_has_split_natives(v: OmniarchiveVersion) -> bool:
    for lib in v.libraries:
...
def main():
    ...
    for filename in os.listdir(os.path.join(UPSTREAM_DIR, VERSIONS_DIR)):
        ...
        omniarchive_version = OmniarchiveVersion.parse_file(input_file)
        v = omniarchive_version.to_meta_version(
            "Minecraft", MINECRAFT_COMPONENT, omniarchive_version.id
        )
        ...
        if not omniarchive_version.minecraft_arguments and omniarchive_version.arguments:
            v.minecraft_arguments = adapt_new_style_arguments(omniarchive_version.arguments)
            if not v.additional_traits:
                v.additional_traits = []
            v.additional_traits.extend(
                adapt_new_style_arguments_to_traits(omniarchive_version.arguments)
            )
    ...
    omniarchive_index = OmniarchiveIndexWrap(
        OmniarchiveIndex.parse_file(os.path.join(UPSTREAM_DIR, VERSION_MANIFEST_FILE))
    )

    minecraft_package = MetaPackage(uid=MINECRAFT_COMPONENT, name="Minecraft")
    minecraft_package.recommended = [omniarchive_index.latest.release]
